...
final class GitPortablePath {
    ...
    private static final Set<String> INTERNAL_ROOT_NAMES = Set.of(
            ".worldarchive",
            GitBackupBackend.MANIFEST_PATH.toLowerCase(Locale.ROOT),
            ".worldarchive.restore.lock");

    private GitPortablePath() {
    }
    ...
    static String validate(String value) {
        if (value == null
            ...
            throw new IllegalArgumentException("Git snapshot path is not portable");
        }
    ...
    static void requireNotInternalRoot(String value) {
        String rootName = value.split("/", 2)[0].toLowerCase(Locale.ROOT);
        if (INTERNAL_ROOT_NAMES.contains(rootName)) {
            throw new IllegalArgumentException("Git snapshot path uses an internal root name");
        }
    }

    static Path resolveInside(Path root, String portable) throws GitStorageException {
        String validated;
        try {
            validated = validate(portable);
        } catch (IllegalArgumentException exception) {
            throw new GitStorageException("Git snapshot contains an unsafe path", exception);
        }
        Path result = root;
        for (String segment : validated.split("/")) {
            result = result.resolve(segment);
        }
        result = result.normalize();
        if (!result.startsWith(root) || result.equals(root)) {
            throw new GitStorageException("Git snapshot path escapes its managed directory");
        }
        return result;
    }
    ...
    private static void validateSegment(String segment) {
        if (segment.isEmpty()
            ...
            throw new IllegalArgumentException("Git snapshot path contains an unsafe segment");
        }
        String base = segment.split("\\.", 2)[0].toUpperCase(Locale.ROOT);
        if (WINDOWS_DEVICES.contains(base) || segment.equalsIgnoreCase(".git")) {
            throw new IllegalArgumentException("Git snapshot path uses a reserved name");
        }
    }
}
